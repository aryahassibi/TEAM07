...
const BestSellingProducts = () => {
  const products = [
    ...
    { id: 2, name: "Compresso Ethiopia 500", rating: 5.0, reviewcount: 216, price: 200, image: "./product2.png" },
      ...
      <div className="bestselling-list">
        {products.map((product) => (
          <div className="bestselling-product-card" key={product.id}>
            <a href="#" onClick={(event) => event.preventDefault()} className="bestselling-image-link" aria-label={product.name}>
              <div
            ...
            <button className="bestselling-add-button" aria-label={`Add ${product.name} to cart`}>+</button>
          </div>
        ))}
      </div>
